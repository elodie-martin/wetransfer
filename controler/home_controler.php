...
function upload(){
    ...
    uploadFile($name, $type, $size, $tmp_name, $error);   //Upload du fichier sur le serveur
    updateDb($name, $extension_upload, $message, $id, $size, $date_up, $mailExpe, $expediteur, $mailDesti);     //Mise à jour de la base de données
    envoiMail($number, $emailExpediteur, $emailDestinataire, $message, $size, $fichier, $submit);    //Envoie du mail

    //affichage de la page d'information
    echo $twig->render('info.twig', array('info'=>$info, 'erreur'=>$erreur));

    function uploadFile($name, $type, $size, $tmp_name, $error){
   ...
   function envoiMail($number, $emailExpediteur, $emailDestinataire, $message, $size, $fichier, $submit){
